...
export async function POST(request: NextRequest) {
  try {
    ...
    const file = formData.get('activityImage') as File;
    ...
    // Validate file size (max 10MB for activity images)
    ...
    const result = await new Promise((resolve, reject) => {
      cloudinary.uploader.upload_stream(
        {
          folder: 'activities',
          public_id: `activity_${user.userId}_${Date.now()}`,
          transformation: [
    ...
    return NextResponse.json({
      success: true,
      message: 'Tải ảnh hoạt động thành công',
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Xóa ảnh hoạt động thành công',
      result
    });

  } catch (error: any) {
    console.error('Activity image delete error:', error);
